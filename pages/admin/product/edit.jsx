...
const EditProductPage = () => {
  ...
  const getProductDeatails = async (productId) => {
    try {
      ...
      if (resp.data) {
        ...
        formik.setFieldValue('arimageurl', data.arimageurl);
        formik.setFieldValue('thumbnail', data.thumbnail);
        getCategoryList();
        getBrandtList();
        GetAWSCredentials();

        const productImage = data.images.filter((item) => {
          return item.type === 'normal' || item.type === 'image';
        });

        productImage.map((data) => {
          data.fileUrl = common.imageUrl + data.url;
        });

        const threeSixtyImage = data.images.filter((item) => {
          return item.type === '360Image' || item.type === '360';
        });

        threeSixtyImage.map((data) => {
          data.fileUrl = common.imageUrl + data.url;
        });

        setImageData({ ...imageData, productImage: productImage, threeSixtyImage: threeSixtyImage });

        console.log(productImage);

        if (data.category) {
          getSubcategoryList(data?.category?.categoryName);
        }
      }
  ...
  const onSubmitClick = async (mode = 'add', arimageurl = null, thumbnail = null) => {
    const images = [];
    ...
    let product = {
      ...
      userid: '',
      arimageurl: arimageurl !== null ? arimageurl : formik.values.arimageurl,
      arimagedata: '',
      images: images,
      ...
      thumbnail: thumbnail !== null ? thumbnail : formik.values.thumbnail,
    };
  ...
  const onSpecificationEditClick = ({ id, spec }) => {
  ...
  const onChangeThumbnails = async (data) => {
    formik.setFieldValue('thumbnail', data.fileName);
    await onSubmitClick('add', formik.values.arimageurl, data.fileName);
  };
                    ...
                    <Form onSubmit={formik.handleSubmit}>
                      <Tabs defaultActiveKey='name' id='uncontrolled-tab' className='mb-3 nav-fill'>
                        ...
                        <Tab eventKey='thumbnails' title={'Product Thumbnails'}>
                          <ImageUpload
                            info={{ type: 'image', url: commonService.productThumbUrl + formik.values.thumbnail }}
                              ...
                              <Card.Body>
                                ...
                                {mode === 'Add' ? (
                                  <Form.Group controlId='submitButton' className='text-center mt-5'>
                                    <Button variant='primary' size='sm' style={{ width: '120px' }} onClick={(e) => specificationClick(e)}>
                                      Add
                                    </Button>
                                    <Button variant='danger' className='ms-2 text-white' size='sm' style={{ width: '120px' }} onClick={reset}>
                                      Cancel
                                    </Button>
                                  </Form.Group>
                                ) : (
                                  <Form.Group controlId='submitButton' className='text-center mt-5'>
                                    <Button variant='primary' size='sm' style={{ width: '120px' }} onClick={onUpdateClick}>
                                      Update
                                    </Button>
                                    <Button variant='danger' className='ms-2 text-white' size='sm' style={{ width: '120px' }} onClick={reset}>
                                      Cancel
                            ...
                            formik.values.specifications.map((data, i) => (
                              <Row key={i} className='mb-2'>
                                <Col>
                                  <Card>
                                    <Card.Body>
                                      <div className='d-inline-block'>{data?.spec}</div>
                                      <div className='d-inline-block float-end'>
                                        <i
                                          className='fa fa-edit me-2'
                                          onClick={() => onSpecificationEditClick(data)}
                                          style={{ cursor: 'pointer' }}
                                        ></i>
                                        <i className='fa fa-trash' onClick={() => onDeleteClick(data)} style={{ cursor: 'pointer' }}></i>
                                      </div>
                                    </Card.Body>
                                  </Card>
                                </Col>
